...
class MySQLHandlerAgent:
    """
    Agent responsible for creating MySQL tables with user_id columns
    and loading CSV data into the tables with the appropriate user_id.
    """
    
    def __init__(self, 
                db_url: str = "mysql+pymysql://root:root@localhost:3306/parseqri", 
                schema: str = "parseqri"):
        """
        Initialize the MySQL Handler Agent.
    ...
    def process(self, context: QueryContext) -> AgentResponse:
        """Process the context for MySQL operations"""
        try:
            if not context.user_id:
                return AgentResponse(
                    success=False,
                    message="User ID is required for MySQL operations",
                    data={}
                )
            ...
            if hasattr(context, 'csv_file') and context.csv_file:
                ...
                return AgentResponse(
                    success=True,
                    message=f"Data loaded successfully into MySQL table {table_name}",
            ...
            return AgentResponse(
                success=True,
                message="No specific MySQL operation required",
                data={}
            )
    ...
    def create_and_populate_table(self, user_id: str, csv_path: str, 
        ...
        try:
            ...
            # Clean table name - ensure it's valid for MySQL
            ...
            # Truncate if needed (MySQL has a 64 character limit on identifiers)
            qualified_table_name = qualified_table_name[:64]
            
            print(f"Creating table with name: {qualified_table_name}")
    ...
    def _clean_table_name(self, table_name: str) -> str:
        """Clean table name for MySQL compatibility"""
